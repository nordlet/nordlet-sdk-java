/**
 * This file was auto-generated by Fern from our API Definition.
 */

import core.ClientOptions;
import core.Suppliers;
import java.util.function.Supplier;
import resources.account.AsyncAccountClient;
import resources.agreements.AsyncAgreementsClient;
import resources.assets.AsyncAssetsClient;
import resources.audit.AsyncAuditClient;
import resources.bank.AsyncBankClient;
import resources.cash.AsyncCashClient;
import resources.catalog.AsyncCatalogClient;
import resources.consolidation.AsyncConsolidationClient;
import resources.declarations.AsyncDeclarationsClient;
import resources.ecommerce.AsyncEcommerceClient;
import resources.files.AsyncFilesClient;
import resources.hr.AsyncHrClient;
import resources.inventory.AsyncInventoryClient;
import resources.ledger.AsyncLedgerClient;
import resources.partners.AsyncPartnersClient;
import resources.payroll.AsyncPayrollClient;
import resources.pos.AsyncPosClient;
import resources.production.AsyncProductionClient;
import resources.public_.AsyncPublicClient;
import resources.purchases.AsyncPurchasesClient;
import resources.reference.AsyncReferenceClient;
import resources.reports.AsyncReportsClient;
import resources.sales.AsyncSalesClient;
import resources.transport.AsyncTransportClient;
import resources.webhooks.AsyncWebhooksClient;

public class AsyncNordletApiClient {
  protected final ClientOptions clientOptions;

  protected final Supplier<AsyncReferenceClient> referenceClient;

  protected final Supplier<AsyncPartnersClient> partnersClient;

  protected final Supplier<AsyncCatalogClient> catalogClient;

  protected final Supplier<AsyncSalesClient> salesClient;

  protected final Supplier<AsyncPurchasesClient> purchasesClient;

  protected final Supplier<AsyncDeclarationsClient> declarationsClient;

  protected final Supplier<AsyncLedgerClient> ledgerClient;

  protected final Supplier<AsyncAssetsClient> assetsClient;

  protected final Supplier<AsyncHrClient> hrClient;

  protected final Supplier<AsyncPayrollClient> payrollClient;

  protected final Supplier<AsyncAgreementsClient> agreementsClient;

  protected final Supplier<AsyncInventoryClient> inventoryClient;

  protected final Supplier<AsyncProductionClient> productionClient;

  protected final Supplier<AsyncEcommerceClient> ecommerceClient;

  protected final Supplier<AsyncCashClient> cashClient;

  protected final Supplier<AsyncTransportClient> transportClient;

  protected final Supplier<AsyncPosClient> posClient;

  protected final Supplier<AsyncAuditClient> auditClient;

  protected final Supplier<AsyncWebhooksClient> webhooksClient;

  protected final Supplier<AsyncBankClient> bankClient;

  protected final Supplier<AsyncFilesClient> filesClient;

  protected final Supplier<AsyncReportsClient> reportsClient;

  protected final Supplier<AsyncConsolidationClient> consolidationClient;

  protected final Supplier<AsyncPublicClient> publicClient;

  protected final Supplier<AsyncAccountClient> accountClient;

  public AsyncNordletApiClient(ClientOptions clientOptions) {
    this.clientOptions = clientOptions;
    this.referenceClient = Suppliers.memoize(() -> new AsyncReferenceClient(clientOptions));
    this.partnersClient = Suppliers.memoize(() -> new AsyncPartnersClient(clientOptions));
    this.catalogClient = Suppliers.memoize(() -> new AsyncCatalogClient(clientOptions));
    this.salesClient = Suppliers.memoize(() -> new AsyncSalesClient(clientOptions));
    this.purchasesClient = Suppliers.memoize(() -> new AsyncPurchasesClient(clientOptions));
    this.declarationsClient = Suppliers.memoize(() -> new AsyncDeclarationsClient(clientOptions));
    this.ledgerClient = Suppliers.memoize(() -> new AsyncLedgerClient(clientOptions));
    this.assetsClient = Suppliers.memoize(() -> new AsyncAssetsClient(clientOptions));
    this.hrClient = Suppliers.memoize(() -> new AsyncHrClient(clientOptions));
    this.payrollClient = Suppliers.memoize(() -> new AsyncPayrollClient(clientOptions));
    this.agreementsClient = Suppliers.memoize(() -> new AsyncAgreementsClient(clientOptions));
    this.inventoryClient = Suppliers.memoize(() -> new AsyncInventoryClient(clientOptions));
    this.productionClient = Suppliers.memoize(() -> new AsyncProductionClient(clientOptions));
    this.ecommerceClient = Suppliers.memoize(() -> new AsyncEcommerceClient(clientOptions));
    this.cashClient = Suppliers.memoize(() -> new AsyncCashClient(clientOptions));
    this.transportClient = Suppliers.memoize(() -> new AsyncTransportClient(clientOptions));
    this.posClient = Suppliers.memoize(() -> new AsyncPosClient(clientOptions));
    this.auditClient = Suppliers.memoize(() -> new AsyncAuditClient(clientOptions));
    this.webhooksClient = Suppliers.memoize(() -> new AsyncWebhooksClient(clientOptions));
    this.bankClient = Suppliers.memoize(() -> new AsyncBankClient(clientOptions));
    this.filesClient = Suppliers.memoize(() -> new AsyncFilesClient(clientOptions));
    this.reportsClient = Suppliers.memoize(() -> new AsyncReportsClient(clientOptions));
    this.consolidationClient = Suppliers.memoize(() -> new AsyncConsolidationClient(clientOptions));
    this.publicClient = Suppliers.memoize(() -> new AsyncPublicClient(clientOptions));
    this.accountClient = Suppliers.memoize(() -> new AsyncAccountClient(clientOptions));
  }

  public AsyncReferenceClient reference() {
    return this.referenceClient.get();
  }

  public AsyncPartnersClient partners() {
    return this.partnersClient.get();
  }

  public AsyncCatalogClient catalog() {
    return this.catalogClient.get();
  }

  public AsyncSalesClient sales() {
    return this.salesClient.get();
  }

  public AsyncPurchasesClient purchases() {
    return this.purchasesClient.get();
  }

  public AsyncDeclarationsClient declarations() {
    return this.declarationsClient.get();
  }

  public AsyncLedgerClient ledger() {
    return this.ledgerClient.get();
  }

  public AsyncAssetsClient assets() {
    return this.assetsClient.get();
  }

  public AsyncHrClient hr() {
    return this.hrClient.get();
  }

  public AsyncPayrollClient payroll() {
    return this.payrollClient.get();
  }

  public AsyncAgreementsClient agreements() {
    return this.agreementsClient.get();
  }

  public AsyncInventoryClient inventory() {
    return this.inventoryClient.get();
  }

  public AsyncProductionClient production() {
    return this.productionClient.get();
  }

  public AsyncEcommerceClient ecommerce() {
    return this.ecommerceClient.get();
  }

  public AsyncCashClient cash() {
    return this.cashClient.get();
  }

  public AsyncTransportClient transport() {
    return this.transportClient.get();
  }

  public AsyncPosClient pos() {
    return this.posClient.get();
  }

  public AsyncAuditClient audit() {
    return this.auditClient.get();
  }

  public AsyncWebhooksClient webhooks() {
    return this.webhooksClient.get();
  }

  public AsyncBankClient bank() {
    return this.bankClient.get();
  }

  public AsyncFilesClient files() {
    return this.filesClient.get();
  }

  public AsyncReportsClient reports() {
    return this.reportsClient.get();
  }

  public AsyncConsolidationClient consolidation() {
    return this.consolidationClient.get();
  }

  public AsyncPublicClient public_() {
    return this.publicClient.get();
  }

  public AsyncAccountClient account() {
    return this.accountClient.get();
  }

  public static AsyncNordletApiClientBuilder builder() {
    return new AsyncNordletApiClientBuilder();
  }
}
