/**
 * This file was auto-generated by Fern from our API Definition.
 */

package com.nordlet.api.resources.inventory.requests;

import com.fasterxml.jackson.annotation.JsonAnyGetter;
import com.fasterxml.jackson.annotation.JsonAnySetter;
import com.fasterxml.jackson.annotation.JsonIgnoreProperties;
import com.fasterxml.jackson.annotation.JsonInclude;
import com.fasterxml.jackson.annotation.JsonProperty;
import com.fasterxml.jackson.annotation.JsonSetter;
import com.fasterxml.jackson.annotation.Nulls;
import com.fasterxml.jackson.databind.annotation.JsonDeserialize;
import com.nordlet.api.core.ObjectMappers;
import java.lang.Object;
import java.lang.String;
import java.util.HashMap;
import java.util.Map;
import java.util.Objects;
import java.util.Optional;
import org.jetbrains.annotations.NotNull;

@JsonInclude(JsonInclude.Include.NON_ABSENT)
@JsonDeserialize(
    builder = PostV1InventoryStockReceiveRequest.Builder.class
)
public final class PostV1InventoryStockReceiveRequest {
  private final String warehouseId;

  private final String itemId;

  private final String date;

  private final String quantity;

  private final String unitCost;

  private final Optional<String> notes;

  private final Map<String, Object> additionalProperties;

  private PostV1InventoryStockReceiveRequest(String warehouseId, String itemId, String date,
      String quantity, String unitCost, Optional<String> notes,
      Map<String, Object> additionalProperties) {
    this.warehouseId = warehouseId;
    this.itemId = itemId;
    this.date = date;
    this.quantity = quantity;
    this.unitCost = unitCost;
    this.notes = notes;
    this.additionalProperties = additionalProperties;
  }

  @JsonProperty("warehouseId")
  public String getWarehouseId() {
    return warehouseId;
  }

  @JsonProperty("itemId")
  public String getItemId() {
    return itemId;
  }

  @JsonProperty("date")
  public String getDate() {
    return date;
  }

  @JsonProperty("quantity")
  public String getQuantity() {
    return quantity;
  }

  @JsonProperty("unitCost")
  public String getUnitCost() {
    return unitCost;
  }

  @JsonProperty("notes")
  public Optional<String> getNotes() {
    return notes;
  }

  @java.lang.Override
  public boolean equals(Object other) {
    if (this == other) return true;
    return other instanceof PostV1InventoryStockReceiveRequest && equalTo((PostV1InventoryStockReceiveRequest) other);
  }

  @JsonAnyGetter
  public Map<String, Object> getAdditionalProperties() {
    return this.additionalProperties;
  }

  private boolean equalTo(PostV1InventoryStockReceiveRequest other) {
    return warehouseId.equals(other.warehouseId) && itemId.equals(other.itemId) && date.equals(other.date) && quantity.equals(other.quantity) && unitCost.equals(other.unitCost) && notes.equals(other.notes);
  }

  @java.lang.Override
  public int hashCode() {
    return Objects.hash(this.warehouseId, this.itemId, this.date, this.quantity, this.unitCost, this.notes);
  }

  @java.lang.Override
  public String toString() {
    return ObjectMappers.stringify(this);
  }

  public static WarehouseIdStage builder() {
    return new Builder();
  }

  public interface WarehouseIdStage {
    ItemIdStage warehouseId(@NotNull String warehouseId);

    Builder from(PostV1InventoryStockReceiveRequest other);
  }

  public interface ItemIdStage {
    DateStage itemId(@NotNull String itemId);
  }

  public interface DateStage {
    QuantityStage date(@NotNull String date);
  }

  public interface QuantityStage {
    UnitCostStage quantity(@NotNull String quantity);
  }

  public interface UnitCostStage {
    _FinalStage unitCost(@NotNull String unitCost);
  }

  public interface _FinalStage {
    PostV1InventoryStockReceiveRequest build();

    _FinalStage additionalProperty(String key, Object value);

    _FinalStage additionalProperties(Map<String, Object> additionalProperties);

    _FinalStage notes(Optional<String> notes);

    _FinalStage notes(String notes);
  }

  @JsonIgnoreProperties(
      ignoreUnknown = true
  )
  public static final class Builder implements WarehouseIdStage, ItemIdStage, DateStage, QuantityStage, UnitCostStage, _FinalStage {
    private String warehouseId;

    private String itemId;

    private String date;

    private String quantity;

    private String unitCost;

    private Optional<String> notes = Optional.empty();

    @JsonAnySetter
    private Map<String, Object> additionalProperties = new HashMap<>();

    private Builder() {
    }

    @java.lang.Override
    public Builder from(PostV1InventoryStockReceiveRequest other) {
      warehouseId(other.getWarehouseId());
      itemId(other.getItemId());
      date(other.getDate());
      quantity(other.getQuantity());
      unitCost(other.getUnitCost());
      notes(other.getNotes());
      return this;
    }

    @java.lang.Override
    @JsonSetter("warehouseId")
    public ItemIdStage warehouseId(@NotNull String warehouseId) {
      this.warehouseId = Objects.requireNonNull(warehouseId, "warehouseId must not be null");
      return this;
    }

    @java.lang.Override
    @JsonSetter("itemId")
    public DateStage itemId(@NotNull String itemId) {
      this.itemId = Objects.requireNonNull(itemId, "itemId must not be null");
      return this;
    }

    @java.lang.Override
    @JsonSetter("date")
    public QuantityStage date(@NotNull String date) {
      this.date = Objects.requireNonNull(date, "date must not be null");
      return this;
    }

    @java.lang.Override
    @JsonSetter("quantity")
    public UnitCostStage quantity(@NotNull String quantity) {
      this.quantity = Objects.requireNonNull(quantity, "quantity must not be null");
      return this;
    }

    @java.lang.Override
    @JsonSetter("unitCost")
    public _FinalStage unitCost(@NotNull String unitCost) {
      this.unitCost = Objects.requireNonNull(unitCost, "unitCost must not be null");
      return this;
    }

    @java.lang.Override
    public _FinalStage notes(String notes) {
      this.notes = Optional.ofNullable(notes);
      return this;
    }

    @java.lang.Override
    @JsonSetter(
        value = "notes",
        nulls = Nulls.SKIP
    )
    public _FinalStage notes(Optional<String> notes) {
      this.notes = notes;
      return this;
    }

    @java.lang.Override
    public PostV1InventoryStockReceiveRequest build() {
      return new PostV1InventoryStockReceiveRequest(warehouseId, itemId, date, quantity, unitCost, notes, additionalProperties);
    }

    @java.lang.Override
    public Builder additionalProperty(String key, Object value) {
      this.additionalProperties.put(key, value);
      return this;
    }

    @java.lang.Override
    public Builder additionalProperties(Map<String, Object> additionalProperties) {
      this.additionalProperties.putAll(additionalProperties);
      return this;
    }
  }
}
